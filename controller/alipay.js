 ...
 */
props.signVerify = function (response,callback) {
    var ret = utl.copy(response);
    var sign = ret['sign'];
    ret.sign = undefined;
    ret.sign_type = undefined;
    var tmp = utl.encodeParams(ret);
    utl.signVerify(tmp.unencode,sign,this.rsaPublic, this.signType, function (result) {
        callback(result);
    });
}
 ...
 * @param {Object} opts
 * @param {String} [opts.out_trade_no]    订单支付时传入的商户订单号,和支付宝交易号不能同时为空。 tradeId,outTradeId如果同时存在优先取tradeId
 * @param {String} [opts.trade_no]       支付宝交易号，和商户订单号不能同时为空
 * @param {String} [opts.appAuthToken]  https://doc.open.alipay.com/doc2/detail.htm?treeId=216&articleId=105193&docType=1
 */
props.query = function (opts,callback) {
    var biz_content = {
        out_trade_no: opts.out_trade_no,
        trade_no: opts.trade_no
    };
    ...
    //获取客户端发送的数据后，拿此数据向支付宝发起验证(向支付宝发送验证请求)
    request(config, function(err, res, body){
        //body为支付宝验证后回传的数据
        console.log("支付宝的返回值："+body);
